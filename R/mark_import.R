...
#' @param na_values Replace specified characters to NA, read.csv->na.strings
#' @param na_drop Skip error evaluation of lines where control-column value is empty
#' @examples 
#' \dontrun{
#' mark_transform(file='maculinea_2021.07.19.csv')
#' }
#' @export
mark_transform <- function(file=NULL, data=NULL, csv.sep=NULL, csv.quote=NULL, write_output_files=T, na_values="", na_drop=F) {
    # default values 
    csv.file <- file
    ...
    x_mark_transform(csv.data,id_col,date_col,species_name_col,sex_col,comment_col,write_output_files,na_drop=F)
}
...
#' @param na_drop drop na? - skip error evaluation of lines where control-column value is empty
#' @keywords Mark transform error-check

x_mark_transform <- function(data,id_col,date_col,species_name_col=NULL,sex_col=NULL,comment_col=NULL,write_output_files=T,na_drop=F) {
