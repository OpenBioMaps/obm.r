...
#' @importFrom utils count.fields edit read.csv2 write.csv zip
#' @export
create_table <- function(file=NULL, data=NULL, sep=',', quote="'", createtable=FALSE, project=FALSE, table=NULL, na_values="") {
    ...
    if (file_type == '.csv') {
        ...
        csv.data <- openxlsx::read.xlsx(csv.file, sheet = 1)
...
#' @param na_drop Drop na? default is True
#' @keywords analyse
#' @importFrom stats na.exclude
#' @export
analyse <- function(column,cn,counter,na_drop=T) {
    type <- class( column )
    flev <- length(levels(as.factor(column)))
    ...
    if (type == 'integer') {
        if (min(column,na.rm=T) == 0 && max(column,na.rm=T)==1) {
            type = 'boolean'
            return(type)
        } else {
            if (max(column,na.rm=T) > 32678) {
                return('integer')
            } else {
                return('smallint')
            }
    ...
    if (type == 'factor' || type == 'logical') {
        ...
        if (na_drop==T) {
            column <- droplevels(as.factor(column[column != ""]))
        }
        f.mod <- tryCatch({
            f.mod <- as.character(as.numeric(levels(column)))
        ...
        if (!anyNA(f.mod) && length(f.mod)) {
            ...
            #    }
            #} else {
                y <- as.numeric(as.matrix(column))
                if (!isTRUE(all(y == floor(y)))) { 
                    type <- 'real'
                    return(type)
                } else {

                    if (max(column,na.rm=T) > 32768) {
                        type <- 'integer'
                    } else {
                        type <- 'smallint'
                    }
                ...
                isdate <- tryCatch({
                    isdate <- try(as.Date(na.exclude(column)),silent=T)
                    #if (!anyNA(isdate)){
            ...
            if ( flev == 2 && nchar(levels(as.factor(column))[1])<12 ) {
                print(paste('Probably logical type: ',cn,' (',levels(as.factor(column)),')',sep=''))
            } else if (flev == 0) {
            ...
            column.length <- tryCatch({
                column.length <- max(sapply(as.character(column),nchar),na.rm=T)
            }, error = function(err) {
            ...
            if (column.length<100) {
                ...
                type <- paste('character varying(',column.length,')',sep='')
                return(type)
            } else {
                type <- 'text'
                return(type)
